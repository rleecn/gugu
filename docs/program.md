...
```go
// 挂起 TUI（退出 raw mode / alt screen），返回 SuspendMsg
cmd := p.Suspend()

// 恢复挂起的 TUI（重新进入 raw mode / alt screen），返回 ResumeMsg
cmd := p.Resume()

// 挂起 → 执行外部命令 → 恢复 → 返回 ExecDoneMsg{Stdout, Stderr, Err}
cmd := p.Exec("less", "/etc/hosts")

// 类似 Exec，但接受自定义 *exec.Cmd
cmd := p.ExecCommand(cmd)
```
